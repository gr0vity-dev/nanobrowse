...
async def fetch_account_info(account):
    ...
    tasks = {
        "account_info": nanorpc.account_info(
            account,
...
async def transform_account_data(data):
    ...
    is_known_account, known_account = await account_lookup.lookup_account(main_account)
    is_known_representative, known_representative = await account_lookup.lookup_account(
        representative
    )

    representative_formatted = (
        known_representative["name"]
        if is_known_representative
        else format_account(representative)
    )
    account_formatted = (
        known_account["name"] if is_known_account else format_account(main_account)
    )
    ...
    response = {
        ...
        "confirmed_balance": format_balance(
            account_info.get("confirmed_balance", 0), "any"
        ),
        ...
        "unconfirmed_blocks": int(account_info.get("block_count", 0))
        - int(account_info.get("confirmed_height", 0)),
    ...
    response["show_receivable"] = True if int(response["receivable"]) > 0 else False
    response["show_unconfirmed_count"] = (
        True if int(response["unconfirmed_blocks"]) > 0 else False
    )
    response["show_weight"] = show_weight
    response["is_pr"] = (
        True if response["show_weight"] and weight_percent > 0.1 else False
    )
