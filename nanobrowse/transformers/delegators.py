...
async def transform_delegator_data(delegators, base_weight):
    ...
    if not delegators or delegators == ""  : return []
    # Sort the raw delegators data to get top 50 by weight
    top_50_delegators = sorted(delegators.items(), key=lambda x: int(x[1]), reverse=True)[:50]
    ...
    for delegator, weight in top_50_delegators:
        ...
        formatted_weight, weight_percent, show_weight = format_weight(weight, base_weight)
